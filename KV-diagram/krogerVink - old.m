...
global k temperature dielectric_const E_MK ...
    ...
    total_dos energy efermi cellsize;
...
function y = nc(mu_e)
global total_dos energy cbm vbm
step = energy(2)-energy(1);
y = sum(total_dos(energy >= cbm).* step .* ...
    fermi_dirac(energy(energy >= cbm),mu_e+vbm));
end

function y = pv(mu_e)
global total_dos energy vbm
step = energy(2)-energy(1);
y = sum(total_dos(energy <= vbm).* step .* ...
    fermi_dirac(-1*energy(energy <= vbm),-mu_e-vbm));
end