...
% **Obsolete ** 6 = Scan [C] of H at 1 atm across different mu_E
% 6 = Scan [D] of H at 1 atm across different element
...
%allDefectElement = ["Co", "Cr", "Cu", "F", "In", "Mn", "Mo", "Nb", "Ni", "Pd", "Rh",...
%    "Sb", "Ta", "Ti", "V", "W", "Zn", "Zr"];
allDefectElement = ["Co", "Cr", "Cu", "F", "Mn", "Mo", "Rh",...
    "Sb", "Ti", "V", "W", "Zn", "Zr"];
...
if module == 1.5
    ...
    for i = E_O2/2+mu_O_p0:-0.5:-8.4
        ...
        %mu_H = 1000;
        %mu_e_result = double(vpasolve(systemTotalCharge(mu_e) == 0, mu_e, mu_e_range));
        mu_e_result = fzero(@(x)systemTotalCharge(x), 1);
...
if module == 2
    ...
    result = [];
    output = [];
    for temperature = [600,1200]
        for dopingConcentration = [10^-2, 10^-4]
            temperature
            dopingConcentration
            outputFile = 'K-V-'+defectElement+'-'+ temperature + 'K-' + dopingConcentration + '.txt';
            for i = E_O2/2+mu_O_p0:-0.1:-7.4
                mu_O = i;
                pO2 = exp(2*(mu_O-E_O2/2-mu_O_p0)/k/temperature)
                mu_Metal = 1/xM*(E_MetalOxide - yO*mu_O);
                %mu_H = (E_water - mu_O)/2;
...
if module == 2.5
    ...
    output = [];
    %"Co", "Zn", "Cu", "Rh" (1200K), 
    %for defectElement = ["Rh","Ni", "Mn", "Cr", "Pd", "Ti","Zr", "V", "Mo", "W", "Nb"]
     for defectElement = ["F"]
        element = [systemElement, defectElement];
        defectFile = 'defects-'+defectElement+'.txt';
        defect = importdata(defectFile);
        ...
        fclose(fID);
        for temperature = [600,1200]
            for dopingConcentration = [10^-4, 10^-2]
                defectElement
                temperature
                dopingConcentration
                outputFile = 'K-V-'+defectElement+'-'+ temperature + 'K-' + dopingConcentration + '.txt';
                for i = E_O2/2+mu_O_p0:-0.1:log(1e-31)/2*k*temperature+E_O2/2+mu_O_p0
                    mu_O = i;
                    pO2 = exp(2*(mu_O-E_O2/2-mu_O_p0)/k/temperature)
                    mu_Metal = 1/xM*(E_MetalOxide - yO*mu_O);
                    %mu_H = (E_water - mu_O)/2;
...
if module == 5
    ...
    dlmwrite('Hvsmu_E-GLEnames.txt','','delimiter',''); % clear file
    ...
    fprintf("\nOutput at Hvsmu_E.txt and Hvsmu_E-GLEnames.txt\n")
% end
% %%
...
%%% Scan [D] of H across different elements
        ...
        for z = 1:length(temperatureRange)
            ...
            if(isempty(mu_e_result))
                ...
                for j = 19:8:27
                    ...
                    c = concentration(E_form, noSite);
                    if j == 19
                        d = 10^14*c*exp(-0.67/(k*temperature));
                    else 
                        d = 10^14*c*exp(-0.67/(k*temperature))*exp(-E_form/(k*temperature));
                    end
